...
`));
    ...
    async function be() {
      ...
      }) : o ? a(new WebAssembly.Instance(y, G()), y) : (ae ??= e.locateFile ? e.locateFile ? e.locateFile("ort-wasm-simd-threaded.jsep.wasm", m) : m + "ort-wasm-simd-threaded.jsep.wasm" : new URL("ort-wasm-simd-threaded.jsep.wasm", import_meta.url).href, l = await (async function(f) {
        var w = ae;
        if (!g && !z(w)) try {
...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  // _ort_entry_tmp.mjs
  globalThis.ort = ort_bundle_min_exports;
})();
